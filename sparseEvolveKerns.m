%% sparseEvolveKerns
% Evolve kernels competitively using sparse arrays and event-based
% processing
...
%% Algorithm - O(Evolution * Kernels^2 * Convolutions)  % TODO revise
...
data = double(loaded(sizex*2 + 1: sizex*3, :, :));
clearvars loaded % Clean up a little
zeroz = find(data == 0);
events = find(data ~= 0);
%% Madness
[xs, ys, zs] = ind2sub(size(data), events);
arrayfun(@(x, y, z) getGrid(data, x, y, z), xs, ys, zs);
% After array will have a nevetns x kernsize matrix (probs 4D)
% Now page fun can take advanatge of them when doing computes with kernel
...
res = zeros([nkernels + 1, size(data)]); % +1 for mutant
oldChampScores = zeros(nkernels, 1);
prevChampTmp = zeros(size(data));
scores = zeros(nkernels + 1, 1);
mutant_wins = [];
...
% Initialise kernelsdat
for ikernel = 1 : nkernels
    khistory{1, ikernel} = randKern();
